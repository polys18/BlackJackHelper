...
static esp_err_t capture_and_post_handler(httpd_req_t *req)
{
    camera_fb_t *fb = NULL;
    ...
    // Send the frame to your external server
    esp_err_t post_res = post_frame_to_server(fb);

    // Important: return the framebuffer to the driver
    esp_camera_fb_return(fb);

#if ARDUHAL_LOG_LEVEL >= ARDUHAL_LOG_LEVEL_INFO
    int64_t fr_end = esp_timer_get_time();
    log_i("capture_and_post: %ums", (uint32_t)((fr_end - fr_start) / 1000));
#endif
    ...
    const char *resp_str = "OK: frame captured and posted\n";
    httpd_resp_set_type(req, "text/plain");
    return httpd_resp_send(req, resp_str, strlen(resp_str));
}


static esp_err_t post_frame_to_server(camera_fb_t *fb)
{
    if (!fb || !fb->buf || fb->len == 0) {
        log_e("post_frame_to_server: invalid frame buffer");
        return ESP_FAIL;
    }

    HTTPClient http;

    // Change path here if your server expects something else:
    const char *url = "http://192.168.68.52:8000/frame";

    log_i("Posting frame to %s (size: %u bytes)", url, (unsigned int)fb->len);
    ...
    log_i("HTTP POST response code: %d", httpCode);

    // You can read response body if needed:
    // String payload = http.getString();
    // log_i("Server response: %s", payload.c_str());
...
void startCameraServer()
{
    ...
    httpd_uri_t index_uri = {
        ...
        .uri = "/capture_post",
        .method = HTTP_GET,       // You can change to HTTP_POST if you prefer
        .handler = capture_and_post_handler,
        .user_ctx = NULL
    #ifdef CONFIG_HTTPD_WS_SUPPORT
        ,
        .is_websocket = false,
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK)
    {
        ...
        httpd_register_uri_handler(camera_httpd, &capture_post_uri);
...
{
    #if CONFIG_LED_ILLUMINATOR_ENABLED
    // log_i("LED flash is enabled -> CONFIG_LED_ILLUMINATOR_ENABLED = 0");
